...
/**
request.get('http://skip/salt-pillars.json', function(err, res, body) {
  pillars = JSON.parse(body)
  console.log("got pillars")
  for (var minion in pillars) {
    if (pillars[minion].hasOwnProperty('applications')) {
      apps = pillars[minion].applications
...
router.get('/salt-pillars', function(req, res) {
  console.log("sending pillars")
  res.send(pillars)
})
